...
void cpu_exec(volatile uint32_t n) {
	if(temu_state == END) {
        ...
        m_changeui.t_show->append("Program execution has ended. To restart the program, exit TEMU and run again.");
		return;
	}
	temu_state = RUNNING;
	...
	for(; n > 0; n --) {
		...
		while (p != NULL)
		{
            ...
            if(p->expr_val != (int)expr(p->expr, (bool*)1)){
				temu_state = STOP;
				printf("触发监视点%d: %s, 此时表达式的值为: %d\n", p->NO, p->expr, expr(p->expr, (bool*)1));
                QString ls = QString("触发监视点%d: %s, 此时表达式的值为: %d\n").arg(p->NO).arg(p->expr).arg(expr(p->expr, (bool*)1));
